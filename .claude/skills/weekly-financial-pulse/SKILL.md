...
description: Generate the Swapps weekly financial report — cash by account, P&L, Cuentas por Cobrar (A/R), Cuentas por Pagar (A/P), Jeeves spend, Jeeves↔QuickBooks reconciliation, contracted MRR (swapps-app) vs invoiced revenue, and recommendations — covering the week + month-to-date + year-to-date, then post it to the ClickUp "Finance" channel. Use when asked for the weekly financial pulse, "pulso financiero semanal", or a recurring finance report. Requires the quickbooks (read-only), jeeves, swapps-app (read-only), and clickup MCP connections.
---
...
## Non-negotiables
- **Read-only.** Never create/update/delete anything in QuickBooks, Jeeves or swapps-app.
  The only write is one ClickUp chat message at the end.
